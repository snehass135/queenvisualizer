...
async function linkedListRemove(idx){
    ...
    idx = Math.floor(idx)
    let arrow=animationContainer.children.item(idx*2+1)
...
removeForm.addEventListener('submit', (e) => {
    e.preventDefault()
    console.log(Math.floor(linkedlist.length/2))
    if (removeFormValid(Math.floor(linkedlist.length/2), removeError)){
        linkedListRemove(linkedlist.length/2)
    }
})